...
const UploadQRPsicotecnica = () => {
  ...
  const validateFile = (file) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setFileError("Solo se permiten archivos Imagenes (PNG, JPEG, JPG)")
      return false
    }
  ...
  const subirArchivoPsicotecnica = async (file, idUsuario, idConductor) => {
    ...
    try {
      const response = await documentosHttp.post('/documentos/psicotecnica/obtenerQR', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
        transformRequest: (data) => data,
        //withCredentials: true // Asegúrate de enviar las credenciales si es necesario
      });
  ...
  const handleSubmit = async () => {
    if (!selectedFile) {
      setFileError("Por favor, seleccione un archivo PNG, JPEG, JPG para subir.")
      return
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Navegacion titulo={`Subir QR de conductor [${userData.rol_usuario}]`} />
      {/* Main Content */}
          ...
          >
            ...
            {selectedFile ? (
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-500 mt-2">Solo archivos Imagenes (máx. 10MB)</p>
              </div>
            )}
...
export default UploadQRPsicotecnica
